...
function CustomDonationCard({ option }: { option: DonationOption }) {
  const inputId = useId()
    ...
    <div className="card card-donate p-6 text-center">
      ...
      >
        {hasValidAmount && previewAmount
          ...
          : option.buttonText || 'Donate custom amount'}
      </button>
    </div>
  )
}

function FixedDonationCard({ option }: { option: DonationOption }) {
    ...
    <div className="mx-auto grid max-w-6xl grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
      {options.map((option, index) =>
        option.isCustomAmount ? (
          <CustomDonationCard key={`custom-${index}`} option={option} />
        ) : (
          <FixedDonationCard
